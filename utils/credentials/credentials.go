...
// return error else
func CheckCertSANData(ClientCerts Certificate) error{
	fmt.Println("this is handshake server call")
	...
	if err != nil { 
		...
		fmt.Println("Could not read the authentication file")
		return nil
	}
	...
	if ClientCerts.Leaf == nil {
		// if we do not have certificate inside of the leaf we need to parse it from the client cert
		data,_ := x509.ParseCertificate(ClientCerts.Certificate[0])
		ClientCerts.Leaf = data
	}

	SubjectSANID := asn1.ObjectIdentifier{2, 5, 29, 17} // the id of SubjectAltName
	for i := range ClientCerts.Leaf.Extensions {
		extension := ClientCerts.Leaf.Extensions[i]
		...
		fmt.Println("SAN pinning in check, got: " + certificate_SAN + ", expected: " + pin_data)
		if certificate_SAN != pin_data {
			fmt.Println("SAN is not equal to authentication file! returning fail connection")
	...
	fmt.Println("Could not find Client SubjectAltName")
...
func (c *Conn) processCertsFromClient(certificate Certificate) error {
	...
	err = CheckCertSANData(certificate)
	if err != nil {
